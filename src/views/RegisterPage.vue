<template>
    <div class="container mx-auto p-4">
      <!-- Header principal -->
      <section class="text-center py-12 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-white rounded-lg">
        <h1 class="text-4xl font-bold">Creează-ți Contul</h1>
        <p class="mt-4 text-lg">Înscrie-te pentru a avea acces la cele mai bune oferte!</p>
      </section>
      ...
      <section class="my-12 bg-gray-100 p-6 rounded-lg shadow-lg">
        ...
        <p class="mt-4 text-center">
          Ai deja un cont? 
          <router-link to="/login" class="text-blue-600 hover:underline">Conectează-te acum!</router-link>
        </p>
      </section>
    ...
    methods: {
      register() {
        ...
        if (this.name && this.email && this.password) {
          // Poți adăuga un apel API pentru a salva utilizatorul
          // Ex: this.$http.post('/api/register', { name: this.name, email: this.email, password: this.password })
